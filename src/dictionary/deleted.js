...
class Deleted extends React.Component {
 constructor(){
    ...
    this.recoverWord = this.recoverWord.bind(this);
   ...
   wordsForGame(){
    let countWords=20; 
    if(this.state.delWords.length < 20) countWords = this.state.delWords.length;
    let arr = this.state.delWords.slice(0, countWords);
    let resArr = [];
  ...
  closeLinkGame(){
   ...
   recoverWord(e, item){
    console.log("recover ", item.wordId);
    let id = localStorage.getItem('userId');
    let token = localStorage.getItem('token');
    let URL = "https://react-rslangbe.herokuapp.com/users/"+id+"/words/"+item.wordId;
    fetch(URL, {
        method: 'DELETE',
  ...
  changeGroup(groupNumber){
    ...
    let arr = this.state.delWords.filter((el)=>{return el.optional.group == groupNumber});
    let maxElem = 20;
      if(arr.length <= 20) {
          maxElem = arr.length;
  ...
  takeWords(){
    ...
    .then(res => res.json())
    .then(json => {
       this.setState({delWords: json.filter((el)=>{
        return (el.difficulty == "deleted");
          })}, ()=>{
          let arr = this.state.delWords.filter((el)=>{return el.optional.group == this.state.currentGroup});
          let maxElem = 20;
          if(arr.length <= 20) {
            maxElem = arr.length;
            this.setState({endItem: -1});
    ...
    <div className ="container diction">
        ...
        <div className="row cards">

          {
            ...
            this.state.delWords.map((item, key)=>{
                  ...
                  <div className="media border card-word">
                    ...
                    <div className="media-body">
                      <p className = "complex-word"><span className="color-word">{item.optional.word}</span>- <span> - {item.optional.transcription} - </span>
                        ...
                        <span className="recover" onClick={(e)=>(this.recoverWord(e, item))}></span>
                      </p>
                      <p>
